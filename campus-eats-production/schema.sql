...
CREATE TABLE IF NOT EXISTS buildings (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),name VARCHAR(120) NOT NULL UNIQUE,campus_zone VARCHAR(80),est_minutes INTEGER NOT NULL DEFAULT 10,is_active BOOLEAN NOT NULL DEFAULT TRUE,created_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS customers (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),full_name VARCHAR(150) NOT NULL,email VARCHAR(150) UNIQUE,phone VARCHAR(30) NOT NULL UNIQUE,password_hash TEXT NOT NULL,building_id UUID REFERENCES buildings(id),room_number VARCHAR(120),created_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS idx_customers_email ON customers(email); CREATE INDEX IF NOT EXISTS idx_customers_phone ON customers(phone);
...
ALTER TABLE staff ADD COLUMN IF NOT EXISTS email_enabled BOOLEAN NOT NULL DEFAULT TRUE;
ALTER TABLE staff ADD COLUMN IF NOT EXISTS whatsapp_enabled BOOLEAN NOT NULL DEFAULT FALSE;
