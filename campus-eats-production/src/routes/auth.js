...
router.post('/register', async(req,res)=>{
  try{
    ...
    const exists=await pool.query('SELECT id,email_verified_at FROM customers WHERE lower(email)=lower($1) OR phone=$2',[email,phone]);
    if(exists.rowCount) return res.status(409).json({error:'An account with this email or phone already exists',verificationRequired:!exists.rows[0].email_verified_at,customerId:exists.rows[0].id});
    ...
    res.status(201).json({verificationRequired:true,customerId:customer.id,email:customer.email});
...
router.post('/verify', async(req,res)=>{
  try{
    ...
    const r=await pool.query(`SELECT id,full_name,email,phone,email_verified_at,verification_code_hash,
    ...
    const customer=r.rows[0];
    if(customer.email_verified_at) return res.json({token:signToken({id:customer.id,type:'customer'}),customer});
    ...
    const stored=customer.verification_code_hash||''.padEnd(64,'0');
    const ok=stored.length===expected.length && crypto.timingSafeEqual(Buffer.from(expected),Buffer.from(stored));
    if(!ok){
      await pool.query('UPDATE customers SET verification_attempts=verification_attempts+1 WHERE id=$1',[customer.id]);
      return res.status(400).json({error:'Incorrect verification code'});
    }
    const v=await pool.query(`UPDATE customers SET email_verified_at=now(),verification_code_hash=NULL,
      verification_expires_at=NULL,verification_attempts=0 WHERE id=$1
      RETURNING id,full_name,email,phone,email_verified_at,created_at`,[customer.id]);
...
router.post('/resend-verification', async(req,res)=>{
  try{
    const customerId=String(req.body.customerId||'');
    const r=await pool.query(`SELECT id,full_name,email,phone,email_verified_at,verification_sent_at FROM customers WHERE id=$1`,[customerId]);
    if(!r.rowCount) return res.status(404).json({error:'Account not found'});
    const customer=r.rows[0];
    if(customer.email_verified_at) return res.json({alreadyVerified:true});
...
router.post('/login', async(req,res)=>{
  try{
    ...
    if(as==='customer'&&!account.email_verified_at) return res.status(403).json({error:'Please verify your email first',verificationRequired:true,customerId:account.id,email:account.email});
...
router.post('/staff/register',verifyToken,requireStaff('admin'),async(req,res)=>{
  try{
    ...
    const hash=await bcrypt.hash(password,SALT_ROUNDS);
    const r=await pool.query(`INSERT INTO staff(full_name,email,phone,password_hash,role,email_enabled,whatsapp_enabled) VALUES($1,$2,$3,$4,$5,TRUE,FALSE)
