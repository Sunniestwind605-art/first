...
const {configured:pushConfigured}=require('./utils/push');
...
app.use((req,res,next)=>{res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('X-Frame-Options','DENY');res.setHeader('Referrer-Policy','strict-origin-when-cross-origin');res.setHeader('Permissions-Policy','camera=(self), microphone=(), geolocation=()');if(process.env.NODE_ENV==='production')res.setHeader('Strict-Transport-Security','max-age=31536000; includeSubDomains');next();});
...
app.use('/api',apiLimiter);app.use('/api/auth',authLimiter);app.use('/api/staff/invite',inviteLimiter);
app.get('/health',async(_req,res)=>{try{await pool.query('SELECT 1');res.json({ok:true,service:'Campus Eats',database:'ok',pushConfigured:pushConfigured()});}catch(e){res.status(503).json({ok:false,service:'Campus Eats',database:'unavailable'});}});
app.use('/api/catalog',catalogRoutes);app.use('/api/auth',authRoutes);app.use('/api/orders',orderRoutes);app.use('/api/staff',staffRoutes);app.use('/api/push',pushRoutes);app.use('/api/support',supportRoutes);
...
app.use((req,res,next)=>{if(req.path.startsWith('/api/'))return res.status(404).json({error:'Not found'});if(req.method!=='GET'&&req.method!=='HEAD')return next();res.sendFile(path.join(__dirname,'..','public','index.html'));});
app.use((err,req,res,_next)=>{console.error('Unhandled error',err);res.status(500).json({error:'Internal server error'});});
const PORT=process.env.PORT||4000;
async function start(){await applySchema();await seedData();app.listen(PORT,()=>{console.log(`Campus Eats listening on ${PORT}`);startOrderTimeoutCron();});}
