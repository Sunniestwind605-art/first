...
const {configured:emailConfigured}=require('./utils/email');
...
app.get('/health',async(_req,res)=>{try{await pool.query('SELECT 1');res.json({ok:true,service:'Campus Eats',database:'ok',emailConfigured:emailConfigured()});}catch(e){res.status(503).json({ok:false,service:'Campus Eats',database:'unavailable'});}});
app.use('/api/catalog',catalogRoutes);app.use('/api/auth',authRoutes);app.use('/api/orders',orderRoutes);app.use('/api/staff',staffRoutes);
