...
const { notifyCustomerStatus } = require('./notifications');
const TIMEOUT_MINUTES = Number(process.env.ORDER_TIMEOUT_MINUTES || 5);
async function cancelStaleOrders() {
  try {
    const result = await pool.query(`UPDATE orders SET status='cancelled', cancelled_at=now(), cancel_reason='No runner accepted within ${TIMEOUT_MINUTES} minutes'
      WHERE status='pending' AND created_at < now() - ($1 || ' minutes')::interval RETURNING id`, [TIMEOUT_MINUTES]);
    for (const row of result.rows) {
      await pool.query(`INSERT INTO order_status_events(order_id,status) VALUES($1,'cancelled')`,[row.id]);
      notifyCustomerStatus(row.id).catch(console.error);
    }
