(function(){
  ...
  async function getRegistration(){if(!('serviceWorker'in navigator)||!('PushManager'in window))throw new Error('Push notifications are not supported in this browser.');return navigator.serviceWorker.register('/sw.js');}
  async function subscribe(token,staffId){
    if(Notification.permission==='denied')throw new Error('Notifications are blocked. Enable them in your browser/site settings first.');
    const key=await api('/api/push/public-key');if(!key.configured||!key.publicKey)throw new Error('Push notifications are not configured on the server yet.');
    const reg=await getRegistration();let sub=await reg.pushManager.getSubscription();
